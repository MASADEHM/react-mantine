...
/**
 * Format a project number for display
 */
export const formatProjectNumber = (projectNumber: string): string => {
  // Already formatted
  if (projectNumber.includes("-")) return projectNumber;
  
  // Format as DCCJ-YYYY-XXXXXX
  if (projectNumber.length >= 10) {
    return `DCCJ-${projectNumber.slice(0, 4)}-${projectNumber.slice(4)}`;
  }
